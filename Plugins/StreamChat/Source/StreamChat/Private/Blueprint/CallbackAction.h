...
public:
    ...
    static void CreateLatentAction(
        ...
        T& Output,
        TFunctionRef<void(TFunction<void(const T&)> Callback)> Action)
    {
        if (UWorld* World =
                GEngine->GetWorldFromContextObject(WorldContextObject, EGetWorldErrorMode::LogAndReturnNull))
        {
            FLatentActionManager& LatentActionManager = World->GetLatentActionManager();
            if (LatentActionManager.FindExistingAction<TCallbackAction<T>>(LatentInfo.CallbackTarget, LatentInfo.UUID))
            {
                return;
            }

            TCallbackAction<T>* DelegateAction = new TCallbackAction(Output, LatentInfo);
            Action(
                [DelegateAction](const T& InResult)
                {
